...
export default ({ topic }) => {
  ...
  const count = action.count ? action.count.toLocaleString(config.locale) : Number(0).toLocaleString(config.locale)
  return (
    <div className='topic-article-content topic-social'>
      <div className='participants-box'>
        <span>{`${count} ${t(`proposal-article.participant.${action.count === 1 ? 'singular' : 'plural' }`)}`}</span>
      </div>
