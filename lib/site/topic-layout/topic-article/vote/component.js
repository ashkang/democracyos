...
class Vote extends Component {
  ...
  render () {
    ...
    const votesTotalL10n = votesTotal ? votesTotal.toLocaleString(config.locale) : Number(0).toLocaleString(config.locale)
      ...
      <div className='proposal-options topic-article-content'>
        ...
        <div className='votes-cast'>
          <em className='text-muted'>
            {t('proposal-options.votes-cast', { num: votesTotalL10n })}
          </em>
        </div>
        {
      ...
      <div className='results-summary col-sm-6'>
        {
          Object.keys(votes)
            .map(function (v) {
              ...
              let widthL10n = width ? width.toLocaleString(config.locale) : Number(0).toLocaleString(config.locale)
              width = Math.round(width * 100) / 100
              let count = props.count ? props.count.toLocaleString(config.locale) : Number(0).toLocaleString(config.locale)

              return (
                <div className={option.className + ' votes-results'} key={v}>
                  <h5>{t(option.text)}</h5>
                  <span className='percent'>{widthL10n}٪</span>
                  <span className='votes'>
                    {t('proposal-options.vote-item')}
                    - {count} نفر
                  </span>
                </div>
              )
