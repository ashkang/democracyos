...
export default function CommentFooter (props) {
  const score = props.score ? props.score.toLocaleString(config.locale) : Number(0).toLocaleString(config.locale)
  const replies = props.repliesCount ? props.repliesCount.toLocaleString(config.locale) : Number(0).toLocaleString(config.locale)
  return (
