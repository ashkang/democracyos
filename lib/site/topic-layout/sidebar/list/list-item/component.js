...
function CreatedBy (props) {
  ...
  if (participants.length > 0) {
    const cardinality = participants.length === 1 ? 'singular' : 'plural'
    const msg = t('proposal-article.participant.' + cardinality)
    const count = item.patricipantsCount ? item.participantsCount.toLocaleString(config.locale) : Number(0).toLocaleString(config.locale)
    return <span>{`${count} ${msg}`}</span>
  }
